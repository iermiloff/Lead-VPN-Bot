...
def init_db():
    ...
    cursor = conn.cursor()
    # Таблица пользователей: теперь всегда пишем tg_id как основной идентификатор
    cursor.execute('''
    ...
    ''')
    # Таблица заказов: добавлены поля для блокировки менеджерами и кодов статусов
    cursor.execute('''
...
# --- ИСПРАВЛЕННЫЕ ФУНКЦИИ БАЗЫ ДАННЫХ ---

def add_user(tg_id, username, referrer_code):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    
    # Заменяем значение 'None', если у юзера нет никнейма
    username_str = username if username else f"id_{tg_id}"
    parent_referrer = "нет"
    
    if referrer_code and referrer_code != "нет":
        # Ищем пригласителя по его username или по его tg_id
        cursor.execute("SELECT tg_id, username FROM users WHERE username = ? OR tg_id = ?", (str(referrer_code), str(referrer_code)))
        row = cursor.fetchone()
        if row: 
            # Сохраняем в качестве parent_referrer именно юзернейм (если есть) или ID
            parent_referrer = row[1] if row[1] else str(row[0])
...
# --- ИСПРАВЛЕННЫЕ И ДИНАМИЧЕСКИЕ КНОПКИ ---

def main_menu(user_id):
    """Главное меню: строго разделено на админов и клиентов"""
    buttons = []
    ...
    if user_id in ADMIN_IDS:
        ...
        if user_id == MAIN_ADMIN_ID:
            buttons.append([InlineKeyboardButton(text="🧮 [Главный] Расчет выплат", callback_data="admin_start_calc")])
    # Если зашел обычный клиент / партнер
    else:
        ...
        buttons.append([InlineKeyboardButton(text="🤝 Личный кабинет (Партнерка)", callback_data="role_partner")])
...
async def cmd_start(message: types.Message, command: CommandObject, state: FSMContext):
    ...
    welcome_text = (
        "👋 Приветствуем в панели администратора!" if message.from_user.id in ADMIN_IDS
...
# --- СЦЕНАРИЙ КЛИЕНТА (ОТСЛЕЖИВАНИЕ СТАТУСОВ) ---
...
async def client_orders_status(callback: types.CallbackQuery):
    ...
    if not orders:
        return await callback.message.answer("📭 У вас пока нет активных или созданных заявок.")
        
    report = "📋 **Статус ваших заявок:**\n\n"
    # Маппинг красивых статусов для клиента
    status_decor = {
        "new": "⏳ На рассмотрении менеджером",
        "work": "⚙️ В процессе сборки / конфигурации",
        "offer": "📩 Предложение отправлено (проверьте ЛС)",
        "install": "✅ VPN Успешно запущен!",
        "reject": "❌ Отклонено менеджером"
    }
    
    for o_id, t_type, status in orders:
        # Пытаемся определить статус по тексту, если код статуса старый
        ...
        report += f"🔹 **Заявка #{o_id}** ({t_type})\n└ Статус: {icon} {status.strip()}\n\n"
        
    buttons = [[InlineKeyboardButton(text="⬅️ Назад", callback_data="back_to_menu")]]
    await callback.message.answer(report, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
# --- ИНТЕРФЕЙС CRM ДЛЯ МЕНЕДЖЕРОВ (ЗАЩИТА ОТ ПЕРЕХВАТА) ---
...
async def admin_crm_list(callback: types.CallbackQuery):
    """Отображение списка незавершенных заявок для админов"""
    ...
    await callback.message.answer("📂 **Список активных заявок в CRM:**")
    for o_id, t_type, status, username in rows:
        ...
        await callback.message.answer(msg, parse_mode="Markdown", reply_markup=get_crm_keyboard(o_id, status))
...
async def handle_crm_status_change(callback: types.CallbackQuery):
    ...
    # Проверка на перехват заявки другим админом
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    ...
    # Если кто-то уже взял в работу, а текущий админ пытается нажать первую кнопку
    ...
    # Если заявка обрабатывается, но другой админ пытается перехватить действия на следующих этапах
    ...
    # Фиксируем блокировку за админой при взятии в работу
    if action == "work":
    ...
    # Красивое обновление текста в панели админа
    lines = callback.message.text.split("\n")
    clean_lines = [l for l in lines if not l.startswith("📊 Статус:") and not l.startswith("⚙️") and not l.startswith("❌") and not l.startswith("📩") and not l.startswith("🚀")]
...
async def send_admin_alerts(user, target_type, channel, ref_l1, ref_l2, order_id):
    ...
    for admin_id in ADMIN_IDS:
        try: 
            await bot.send_message(chat_id=admin_id, text=admin_alert, reply_markup=markup)
        except Exception: 
            pass
...
async def show_partner_cabinet(message_or_callback, user_id, username, state: FSMContext):
    ...
    if wallet == "не указан":
        text_reg = (
            ...
            "ВАЖНОЕ ПРАВИЛО: Любые формы спам-рассылок категорически запрещены. "
            "Партнеры, уличенные в использовании спама, будут навсегда исключены."
        )
        await target_msg.answer(text_reg)
        ...
        cabinet_text = (
            ...
            f"Ваш TON-кошелек: {wallet}\n\n"
            f"Ваша рекламная ссылка:\n{ref_link}\n\n"
            f"{stats_text}\n\n"
            f"Вы получаете 30% L1 и 10% L2 от чистой прибыли.\n\n"
            f"Спам строго запрещен. При фиксации жалоб - блокировка аккаунта."
        )
        
        buttons = [[InlineKeyboardButton(text="В главное меню", callback_data="back_to_menu")]]
        await target_msg.answer(cabinet_text, reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def admin_start_calculation(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not active_orders: 
        return await callback.message.answer("Нет запущенных VPN-сервисов со статусом Установлен.")
...
async def generate_final_report(message: types.Message, state: FSMContext):
    ...
    for order in queue:
        ...
        details_log += f" {display_name} | Поступило: {incoming_sum:.2f}р\n"
    ...
    for partner, amount in partner_payouts.items():
        payout_sheet += f" {partner} - {amount:.2f} руб.\n"
    ...
    final_report = (
        ...
        f"{details_log}--------------------\n"
        f"{payout_sheet}\n--------------------\n"
        f"Всего вошло: {total_received_money:.2f} руб.\n"
        f"Чистая доля: {total_clean_profit:.2f} руб."
    )
